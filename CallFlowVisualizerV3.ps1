...
function Get-VoiceApp {
    param (
        ...
        elseif ($voiceAppId) {
            $resourceAccount = Get-CsOnlineApplicationInstance | Where-Object {$_.Identity -match $VoiceAppId}
        }
...
function Get-AutoAttendantHolidaysAndAfterHours {
    ...
    $mdAutoAttendant = "autoAttendant$($aaCounter)([Auto Attendant <br> $($aa.Name)])"
...
$mdAutoAttendant --> $nodeElementHolidayCheck
...
"@
    ...
    # Check if auto attendant has no Holidays but after hours
        ...
        if ($aaHasAfterHours -eq $true) {
            ...
            $mdHolidayAndAfterHoursCheck =@"
$mdAutoAttendant --> $nodeElementAfterHoursCheckCheck
$nodeElementAfterHoursCheck -->|No| $mdAutoAttendantAfterHoursCallFlow
$nodeElementAfterHoursCheck -->|Yes| $mdAutoAttendantDefaultCallFlow
...
"@      
        ...
        # Check if auto attendant has no after hours and no holidays
            ...
            $mdHolidayAndAfterHoursCheck =@"
$mdAutoAttendant --> $mdAutoAttendantDefaultCallFlow
    ...
    switch ($voiceAppType) {
        ...
        "Call Queue" {
            ...
            elseif ($NestedCQType -eq "OverFlow") {
                ...
                $voiceAppTypeSpecificCallFlow = "callQueue$($cqCallFlowCounter)([Call Queue <br> $($MatchingCQ.Name)])"
            }
