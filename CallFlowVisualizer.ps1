...
"@
        ...
        } # End of if auto attendants has holidays
        ...
        } # End of else (check if aa has business hours)
...
"@
    ...
    } # End of first switch condition (app Id is = auto attendants)
    ...
    "11cd3e2e-fccb-42ad-ad00-878b93575e07" {
        ...
        # Make sure that these variable are false in case the script is first run for an auto attendant and then a second time for a call queue
        $aaHasHolidays = $false
        $aaHasAfterHours = $false
        ...
        # Get call queue
        $aa = Get-CsCallQueue | Where-Object {$_.ApplicationInstances -eq $resourceAccount.ObjectId}
...
$nodeStart = "start((Incoming Call at <br> $($voiceAppProperties.PhoneNumber)))"
$nodeElementAA = "elementAA([$($voiceAppType) <br> $($voiceAppProperties.DisplayName)])"
